...
gulp.task('build:vendor', [], function () {
    ...
    var streamJs = gulp.src(js).pipe(debug({title: "Vendor JavaScript:"}));
    if (!developmentMode) {
        streamJs = streamJs.pipe(concat("vendor.js"));
    }
    var counter = 10000;
    streamJs = streamJs.pipe(rename(function (path) {
        path.basename = (counter++) + "-" + path.basename;
    }));
    streamJs = streamJs.pipe(gulp.dest(config.targetApp + "/vendor"));
...
gulp.task('build:html', [], function () {
    ...
    s = s.pipe(inject(series(
        //gulp.src(
        //    ["systemjs/system.js", "systemjs/system-polyfills.js"],
